...
export default function ConfirmPage(){
  ...
  const Card = ({children}:{children:React.ReactNode}) => (
    <div style={{width:820, maxWidth:'92vw', background:'rgba(17,24,39,0.78)', border:'1px solid #1f2937', borderRadius:16, padding:18, color:'#e5e7eb', boxShadow:'0 10px 30px rgba(0,0,0,.35)'}}>{children}</div>
  );
  const Helper = ({children}:{children:React.ReactNode}) => (
    <div style={{fontSize:12, color:'#94a3b8', marginTop:6}}>{children}</div>
  );

  return (
    <div style={{minHeight:'100vh', backgroundImage:'url(/images/Background_1.webp)', backgroundSize:'cover', backgroundPosition:'center'}}>
      <UserTopBar />
      <div style={{display:'grid', placeItems:'center', padding:'24px 12px'}}>
        <div style={{display:'grid', placeItems:'center', marginBottom:10}}>
          <Image src="/images/Logo_3.webp" alt="logo" width={160} height={160} style={{opacity:.8}}/>
        </div>
        <Card>
          ...
          {step===1 && (
            <div style={{display:'grid', gap:12}}>
              <label style={{fontSize:14}}>The name of the website where you communicated and conducted transactions</label>
          ...
          {step===6 && (
            <div style={{display:'grid', gap:10}}>
              ...
              <div className="chatbox" style={{whiteSpace:'pre-wrap', background:'#0b1220', border:'1px solid #1f2937', color:'#e5e7eb', borderRadius:8, padding:'10px', minHeight:120}}>
